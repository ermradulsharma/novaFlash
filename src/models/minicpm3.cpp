...
llm_build_minicpm3::llm_build_minicpm3(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    const float scale_embd  = model.hparams.f_embedding_scale;
    const float scale_depth = model.hparams.f_residual_scale;
    ...
    // lm_head scaling
    const float scale_lmhead = model.hparams.f_logit_scale;
