...
llm_build_grovemoe::llm_build_grovemoe(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Precompute GroveMoE expert selection once to avoid redundant argsort and sigmoid calculations
        ggml_tensor * selection_probs = ggml_sigmoid(ctx0, probs);
        cb(selection_probs, "ffn_moe_probs_biased", il);

        ggml_tensor * selected_experts = ggml_argsort_top_k(ctx0, selection_probs, n_expert_used);
        cb(selection_probs, "ffn_moe_argsort", il);
