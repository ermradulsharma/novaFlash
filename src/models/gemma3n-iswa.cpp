...
llm_build_gemma3n_iswa::llm_build_gemma3n_iswa(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        const int64_t n_tokens_l = (il > last_kv_layer && inp_out_ids) ? inp_out_ids->ne[0] : n_tokens;
        ...
        if (hparams.has_kv(il)) {
            ...
            cb(Qcur, "Qcur", il);
            Qcur = ggml_reshape_3d(ctx0, Qcur, n_embd_head, n_head, n_tokens_l);
        ...
        ggml_tensor * first_prediction;                                                   // [n_embd, n_tokens]
        {
            ...
            if (il >= last_kv_layer && inp_out_ids) {
        ...
        // equivalent to python code: corrected_predictions[1:] += first_prediction
        {
            ...
            ggml_tensor * slice_rest  = ggml_view_3d(
                ctx0, corrected, n_embd, n_tokens_l, n_altup - 1, ggml_row_size(corrected->type, n_embd),
                ggml_row_size(corrected->type, n_embd * n_tokens_l), n_embd * n_tokens_l * ggml_element_size(corrected));
        ...
        // strip unused tokens right after the last KV layer to speed up prompt processing
        if (il == last_kv_layer && inp_out_ids) {
    ...
    const int64_t n_tokens_final = inp_out_ids ? inp_out_ids->ne[0] : n_tokens;
    ...
    // cur now has multiple altup(s), we want to merge them back to 1 altup
    {
        ...
        ggml_tensor * alt_slice =
            ggml_view_3d(ctx0, cur, n_embd, n_tokens_final, n_altup - 1, ggml_row_size(cur->type, n_embd),
                         ggml_row_size(cur->type, n_embd * n_tokens_final), n_embd * n_tokens_final * ggml_element_size(cur));
...
ggml_tensor * llm_build_gemma3n_iswa::altup_predict(ggml_tensor * cur, int il) {
    ...
    const int64_t n_tokens_l = cur->ne[1];
    all_coefs = ggml_reshape_3d(ctx0, all_coefs, n_altup, n_altup, n_tokens_l);
...
ggml_tensor * llm_build_gemma3n_iswa::altup_correct(ggml_tensor * predictions, ggml_tensor * activated, int il) {
    ...
    const int64_t n_tokens_l = activated->ne[1];
    ...
    all_coefs = ggml_cont_3d(ctx0, all_coefs, 1, n_tokens_l, n_altup);                           // [1, n_tokens, n_altup]

    innovation              = ggml_repeat_4d(ctx0, innovation, n_embd, n_tokens_l, n_altup, 1);
